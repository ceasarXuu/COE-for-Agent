...
export function ArtifactNode({ data }: NodeProps<GraphNodeRecord & { isSelected?: boolean }>) {
  return (
    <div className={`graph-node graph-node-artifact ${data.isSelected ? 'selected' : ''}`} data-testid={`graph-node-${data.id}`}>
