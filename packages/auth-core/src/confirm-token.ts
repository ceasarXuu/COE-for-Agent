...
export const DEFAULT_CONFIRM_TTL_MS = 120 * 1000;
...
export function verifyConfirmToken(token: string, options: VerifyConfirmTokenOptions): ConfirmTokenClaims {
  ...
  if (
    claims.targetIds.length !== expectedTargetIds.length ||
    claims.targetIds.some((value, index) => value !== expectedTargetIds[index])
  ) {
