...
- canonical 命名写入入口：`investigation.issue.record`、`investigation.issue.resolve`、`investigation.context.register`
...
从 2026-04-16 开始，案件图的主视图做了第一阶段收敛：

- `inquiry`、`symptom`、`gap`、`residual` 在图上统一展示为 `issue`
- `entity` 默认不再作为主图节点展示，而是作为上下文信息保留
- 新的 canonical MCP tool 已提供 `issue/context` 命名；旧的 `inquiry/symptom/gap/residual/entity` tool 仍保留兼容
- 底层事件、命令、表结构仍保持兼容，当前收敛发生在 graph projection 和 console 展示层
...
2. Agent 持续记录问题/现象、Artifact 和 Fact，并把实体作为上下文挂接，而不是把所有推理都写在自由文本里。
